...
import cat.urv.deim.EstructuraDatos.hashMap.*;
 ...
 *
 * Forma usada:
 *  Grafo de adyacencia de listas (datos duplicados).
 */
...
// METODOS ADICIONALES
    ...
    /** Actualiza el valor de un vertice.
     *
     * @param key Clave del vertice
     * @param value Nuevo valor del vertice
     */
    public void actualitzarValor(K key, V value) {
        try {
            NodoVertices vertex = grafo.consultar(key); // obtenir el vertex
            vertex.valor = value; // actualitzar el valor
        } catch (ElementNoTrobat e) {
            e.printStackTrace();
        }
    }
