...
public class HashMapIndirecte<K extends Comparable<K>, V> implements IHashMap<K, V> {
    ...
    public K[] obtenirClaus() {
        // Creamos un ArrayList para almacenar las claves
        ArrayList<K> claves = new ArrayList<>();

        // Recorremos la tabla estática
        for (EntryHashMap<K, V> entrada : tablaHash) {
            // Nos posicionamos en la primera entrada
            EntryHashMap<K, V> actual = entrada;

            // Recorremos las colisiones, si hay
            while (actual != null) {
                claves.add(actual.getKey());
                actual = actual.getSiguiente();
            }
        }

        @SuppressWarnings("unchecked")
        // Convertimos el ArrayList a un array de claves
        K[] arrayClaves = (K[]) claves.toArray();
        return arrayClaves;
    }

    @Override
    public Iterator<V> iterator() { return new IteratorHashMap<>(tablaHash); }
    ...
    @Override
    public float factorCarrega() { return (float)numElements() / midaTaula(); }

    @Override
    public int midaTaula() { return tablaHash.size(); }
